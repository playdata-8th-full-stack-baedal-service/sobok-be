...
public interface SocialLoginControllerDocs {
    ...
    @Operation(summary = "카카오 로그인 콜백", description = "카카오 인증 후 전달된 code로 로그인 및 회원가입 처리 후 로그인 정보를 반환합니다.")
    @ApiResponses({
    ...
    void kakaoCallback(
            @Parameter(description = "카카오 인증 서버에서 전달하는 인가 코드", required = true, in = ParameterIn.QUERY)
            @RequestParam String code,
            HttpServletResponse response
    ) throws java.io.IOException;

    @Operation(summary = "구글 로그인 콜백", description = "구글 인증 후 전달된 code로 로그인 및 회원가입 처리 후 로그인 정보를 반환합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "로그인/회원가입 성공",
                    content = @Content(mediaType = "application/json",
    ...
    void googleCallback(
            @Parameter(description = "구글 인증 서버에서 전달하는 인가 코드", required = true, in = ParameterIn.QUERY)
            @RequestParam String code,
            HttpServletResponse response
    ) throws java.io.IOException;
